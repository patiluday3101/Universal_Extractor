...
def extract(pdf_path):
    ...
    if "No text found in PDF" in extracted_text or "Extraction error" in extracted_text:
        print(f"Skipping extraction for, as PDF has no text.")
        return
    ...
    bill_details = {
        ...
        "Bill_date": re.search(r"Bill Issue Date\s*:?\s*([\d/-]+)", extracted_text).group(1) if re.search(r"Bill Issue Date\s*:?\s*([\d/-]+)", extracted_text) else "Not Found",
        "Due_date": re.search(r"Due Date\s*:?\s*([\d/-]+)", extracted_text).group(1) if re.search(r"Due Date\s*:?\s*([\d/-]+)", extracted_text) else "Not Found",
